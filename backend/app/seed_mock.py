...
def _pos(user_id: str, **kwargs) -> Position:
    ...
    if defaults.get("ppc_ars") == Decimal("0") and defaults.get(
...
def _budget(
    user_id: str, income_ars: Decimal, cats: list[dict]
) -> tuple[BudgetConfig, list[BudgetCategory]]:
    expenses_pct = sum(
        Decimal(str(c["percentage"])) for c in cats if not c.get("is_vacation")
    )
    ...
    return budget, [
        BudgetCategory(
            percentage=Decimal(str(c["percentage"])),
            **{k: v for k, v in c.items() if k != "percentage"},
        )
        for c in cats
    ]
...
def _snapshots(
    user_id: str, base_usd: float, days: int = 60, growth_rate: float = 0.003
...
def _investment_months(
    user_id: str, months: int = 6, amount_ars: float = 500_000
) -> list[InvestmentMonth]:
    result = []
...
def _seed_a(db: Session) -> None:
    ...
    positions = [
        _pos(
            ...
            avg_purchase_price_usd=Decimal("0.45"),
        ...
        _pos(
            uid,
            ticker="GD30",
            description="Bono Soberano GD30 USD",
            asset_type="BOND",
            source="IOL",
            quantity=Decimal("70000"),
            avg_purchase_price_usd=Decimal("0.38"),
        ...
        _pos(
            uid,
            ticker="YCA6O",
            description="YPF ON USD 2026",
            asset_type="ON",
            source="IOL",
            quantity=Decimal("17000"),
            avg_purchase_price_usd=Decimal("0.95"),
...
def _seed_c(db: Session) -> None:
    ...
    positions = [
        _pos(
            uid,
            ticker="S31O5",
            description="LECAP S31O5",
            asset_type="LETRA",
            source="IOL",
            quantity=Decimal("5000000"),
            avg_purchase_price_usd=Decimal("0.000710"),
        ...
        _pos(
            uid,
            ticker="IOLMMA",
            description="IOL Money Market ARS",
            asset_type="FCI",
            source="IOL",
            quantity=Decimal("8000000"),
            avg_purchase_price_usd=Decimal("0.000060"),
        ...
        _pos(
            ...
            avg_purchase_price_usd=Decimal("12.00"),
    ...
    db.add(
        FreedomGoal(
            user_id=uid,
            monthly_savings_usd=Decimal("350"),
...
def _seed_d(db: Session) -> None:
    ...
    positions = [
        _pos(
            ...
            avg_purchase_price_usd=Decimal("10.00"),
            current_price_usd=Decimal("14.20"),
            annual_yield_pct=Decimal("0.10"),
        ...
        _pos(
            uid,
            ticker="VIST",
            description="Vista Energy CEDEAR",
            asset_type="CEDEAR",
            source="IOL",
            quantity=Decimal("500"),
            avg_purchase_price_usd=Decimal("38.00"),
        ...
        _pos(
            ...
            avg_purchase_price_usd=Decimal("55000"),
        ...
        _pos(
            ...
            source="NEXO",
            quantity=Decimal("8"),
            avg_purchase_price_usd=Decimal("2500"),
...
def _seed_e(db: Session) -> None:
    ...
    positions = [
        ...
        _pos(
            ...
            avg_purchase_price_usd=Decimal("0.43"),
            current_price_usd=Decimal("0.51"),
            annual_yield_pct=Decimal("0.09"),
        ...
        _pos(
            ...
            source="IOL",
            quantity=Decimal("2000"),
            avg_purchase_price_usd=Decimal("11.00"),
            current_price_usd=Decimal("14.20"),
            annual_yield_pct=Decimal("0.10"),
            current_value_usd=Decimal("28400"),
        ),
        _pos(
            ...
            avg_purchase_price_usd=Decimal("0.92"),
            current_price_usd=Decimal("0.98"),
            annual_yield_pct=Decimal("0.09"),
    ...
    db.add_all(
        [
            FreedomGoal(
                user_id=uid,
                monthly_savings_usd=Decimal("1500"),
