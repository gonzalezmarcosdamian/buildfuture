...
def _lecap_tir(price_per_100: Decimal, days: int) -> Decimal:
    ...
    tir = (Decimal("100") / price_per_100 - Decimal("1")) * (Decimal("365") / Decimal(str(days)))
...
_BOND_YTM: dict[str, Decimal] = {
    ...
    "AL30":  Decimal("0.17"), "AL30D": Decimal("0.17"),
    "GD30":  Decimal("0.16"),
    "AL35":  Decimal("0.16"), "AL35D": Decimal("0.16"),
    "GD35":  Decimal("0.15"),
    # Largo plazo (> 2035)
    "GD38":  Decimal("0.15"), "AE38":  Decimal("0.15"), "AE38D": Decimal("0.15"),
    "AL41":  Decimal("0.15"), "AL41D": Decimal("0.15"),
    "GD41":  Decimal("0.15"),
    "GD46":  Decimal("0.14"),
    # Bopreales
...
def _fci_market_avg_yield() -> Decimal:
    ...
    try:
        ...
        try:
            ...
            prev_by_name = {f["fondo"].lower(): float(f["vcp"]) for f in r.json() if f.get("vcp")}
        except Exception:
            return Decimal("0.38")
        ...
        logger.info("FCI market avg yield: %.2f%% TNA (%d fondos)", avg * 100, len(yields))
        ...
        try:
            ...
            if new_yield is not None and new_yield != pos.annual_yield_pct:
                ...
                logger.info(
                    ...
                    pos.asset_type, pos.ticker, old, float(new_yield) * 100,
                )
                changed = True
            ...
            logger.warning("yield_updater error en %s %s: %s", pos.asset_type, pos.ticker, e)
...
def _yield_lecap(pos, today: date) -> Decimal | None:
    ...
    logger.info(
        ...
        pos.ticker, maturity, days, float(price_per_100), float(tir) * 100,
    )
    return tir
...
def _yield_fci(pos, market_avg: Decimal) -> Decimal | None:
    ...
    logger.info("FCI %s: TNA=%.2f%% (promedio mercado)", pos.ticker, float(market_avg) * 100)
    return market_avg
